...
# -------------------------------------------------
def need_conversion(in_file, target_sample_rate=DEFAULT_SAMPLE_RATE):
    """
    22kHz 모노를 44.1kHz MP3로 변환해야 하는지 확인.
    이미 MP3 44.1kHz 모노인 경우 변환을 스킵.
    """
    try:
        from pydub.utils import mediainfo
        info = mediainfo(in_file)
        if info is None:
            return True  # 메타데이터 얻기 실패 시 변환 시도
        format_name = info.get('format_name', '').lower()
        sample_rate = int(info.get('sample_rate', 0))
        channels = int(info.get('channels', 0))

        # 22kHz 모노 파일을 44.1kHz로 변환해야 함
        if format_name == 'mp3' and sample_rate == target_sample_rate and channels == 1:
            return False  # 변환할 필요 없음

        return True  # 변환 필요
    except Exception as e:
        print(f"Error checking file {in_file}: {e}")
        return True
...
def convert_audio_file(task):
    ...
    # 변환이 필요한지 확인 (이미 모노 22kHz MP3라면 44.1kHz로 변환)
    if not need_conversion(in_file, target_sample_rate=sample_rate):
    ...
    ffmpeg_cmd = [
        ...
        "-i", in_file,
        "-ac", "1",  # 모노로 설정
        "-ar", str(sample_rate),  # 목표 샘플 레이트 44.1kHz
        "-af", "aresample=resampler=soxr:precision=5",  # 속도를 높이기 위한 낮은 precision 리샘플링
        "-f", "mp3",  # MP3 형식으로 변환
        out_file
    ]
    try:
